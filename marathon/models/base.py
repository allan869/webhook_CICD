...
class MarathonObject(object):
    ...
    def json_repr(self, minimal=False):
        ...
        """
        if minimal:
            return {to_camel_case(k): v for k, v in list(vars(self).items()) if (v or v is False or v == 0)}
        else:
            return {to_camel_case(k): v for k, v in list(vars(self).items())}
...
def assert_valid_path(path):
    ...
    for id in [_f for _f in path.strip('/').split('/') if _f]:
